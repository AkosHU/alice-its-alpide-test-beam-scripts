...
echo "#Run number;Energy;Chip ID;Irradiation level(0-nonIrradiated,1-2.5e12,2-1e13,3-700krad,4-combined:1e13+700krad);Rate;Vbb;Ithr;Vcasn;Vcasp;Vrst;Vlight;Trigger delay;Acq time; Data (1) or noise (0);Efficiency (0);Number of tracks (0);Number of tracks with associated hit (0);Efficiency(1);Number of tracks(1);Number of tracks with associated hit(1);Efficiency(2);Number of tracks(2);Number of tracks with associated hit(2);Efficiency(3);Number of tracks(3);Number of tracks with associated hit(3);Config Filename;Event Count;Tracks Alignment;Tracks Fitted;Minimum Timestamp" > ${resultsTable}
...
do
    ...
    do
        ...
        fakeHitRate[${index}]=$(echo $l | cut -f5 -d' ')
        fakeHitRateWTrk[${index}]=$(echo $l | cut -f6 -d' ')
        fakeHitRateWTrkCorr[${index}]=$(echo $l | cut -f7 -d' ')
        fakeHitRateWoTrk[${index}]=$(echo $l | cut -f8 -d' ')
    ...
    minTimestamp=$(cat ${outputFolder}/${rStr}/analysis.log | grep "timestamp" | cut -f8 -d' ')
    ...
    do
        ...
        printf ${fakeHitRate[i]}";"${fakeHitRateWTrk[i]}";"${fakeHitRateWTrkCorr[i]}";"${fakeHitRateWoTrk[i]}";" >> ${resultsTable}
    done
    printf ${config}";"${eventCnt}";"${alignmentTracks}";"${fittedTracks}";"${minTimestamp}"\n" >> ${resultsTable}
done
